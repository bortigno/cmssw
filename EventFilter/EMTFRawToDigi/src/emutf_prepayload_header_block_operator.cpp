...
//std::cout << "Unpacking PrePayloadHeader block" << std::endl;
  ...
  if ( (_16bit_word_d & 0x2    ) != 0x0000 ) *_error_status = mtf7::BLOCK_COUNTER_FORMAT; // check if D1  is 0
  if ( (_16bit_word_d & 0x4    ) != 0x0000 ) *_error_status = mtf7::BLOCK_COUNTER_FORMAT; // check if D2  is 0
  if ( (_16bit_word_d & 0x8    ) != 0x0000 ) *_error_status = mtf7::BLOCK_COUNTER_FORMAT; // check if D3  is 0
  if ( (_16bit_word_d & 0x10   ) != 0x0000 ) *_error_status = mtf7::BLOCK_COUNTER_FORMAT; // check if D4  is 0
