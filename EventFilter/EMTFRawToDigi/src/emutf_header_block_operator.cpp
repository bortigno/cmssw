...
if ( (_16bit_word_c & 0xf000) != 0x9000 ) { *_error_status = mtf7::BLOCK_COUNTER_FORMAT;}
if (  _16bit_word_b           != 0x9000 ){ *_error_status = mtf7::BLOCK_COUNTER_FORMAT; }
if ( (_16bit_word_b & 0xfff ) != 0x000 ){ *_error_status = mtf7::BLOCK_COUNTER_FORMAT; }
...
if (*_error_status == mtf7::BLOCK_COUNTER_FORMAT){ 
  ...
  _unpacked_block_event_info -> _l1a  =  _16bit_word_d & 0xfff;
  _unpacked_block_event_info -> _l1a |= (_16bit_word_c & 0xfff) << 12;
  _unpacked_block_event_info -> _bxn  =  _16bit_word_a & 0xfff;
  // - - - - - - - - - - - - - - - - - - - - - 
  // 2nd 64-bit word ...
  break_into_abcd_words( *at_ptr); at_ptr++;

    // check format identifiers
  if ( (_16bit_word_d & 0xffff) != 0xa000 ) *_error_status = mtf7::EVENT_RECORD_FORMAT;
  ...
  if ( (_16bit_word_b & 0x800 ) != 0x0000 ) *_error_status = mtf7::EVENT_RECORD_FORMAT;
  if ( (_16bit_word_a & 0xf000) != 0xa000 ) *_error_status = mtf7::EVENT_RECORD_FORMAT;

  if (*_error_status != mtf7::NO_ERROR) return 0;

  _unpacked_block_event_info -> _sp_addr =  _16bit_word_c & 0x1f;
  _unpacked_block_event_info -> _sp_ersv = (_16bit_word_c >> 5) & 0x7;
  _unpacked_block_event_info -> _sp_ts   = (_16bit_word_c >> 8) & 0xf;

  _unpacked_block_event_info -> _wof     = (_16bit_word_b & 0x1); _16bit_word_b >>=1;
  _unpacked_block_event_info -> _osy     = (_16bit_word_b & 0x1); _16bit_word_b >>=1;
  _unpacked_block_event_info -> _bsy     = (_16bit_word_b & 0x1); _16bit_word_b >>=1;
  _unpacked_block_event_info -> _rdy     = (_16bit_word_b & 0x1); _16bit_word_b >>=1;
  _unpacked_block_event_info -> _skip    = (_16bit_word_b & 0x1); _16bit_word_b >>=1;
  _unpacked_block_event_info -> _rpca    = (_16bit_word_b & 0x1); _16bit_word_b >>=1;
  _unpacked_block_event_info -> _spa     = (_16bit_word_b & 0x1); _16bit_word_b >>=1;
  _unpacked_block_event_info -> _ddm     = (_16bit_word_b & 0x1); _16bit_word_b >>=1;

  _unpacked_block_event_info -> _tbin    = _16bit_word_b & 0x7; // IKF todo: size of at_ptr from _tbin and _skip
  
  _unpacked_block_event_info -> _ME1a    = _16bit_word_a & 0x1ff;
...
std::cout << "Checkpoint 7" << std::endl;
  // check format identifiers
  if ( (_16bit_word_d & 0x8000) != 0x8000)            *_error_status = mtf7::EVENT_RECORD_FORMAT;
...
std::cout << "Checkpoint 10" << std::endl;
  if ( (_16bit_word_a & 0x8000) != 0x0000)             *_error_status = mtf7::EVENT_RECORD_FORMAT;
std::cout << "Checkpoint 11" << std::endl;

  if (*_error_status != mtf7::NO_ERROR) return 0;
std::cout << "Checkpoint 12" << std::endl;

  _unpacked_block_event_info -> _ME1b =  _16bit_word_d & 0x1ff; _16bit_word_d >>= 9;
  _unpacked_block_event_info -> _RPC  =  _16bit_word_d & 0x3f;

  _unpacked_block_event_info -> _ME2  =  _16bit_word_c & 0x1ff; _16bit_word_c >>= 9;
  _unpacked_block_event_info -> _RPC |= (_16bit_word_c & 0x3f) << 6;

  _unpacked_block_event_info -> _ME3  =  _16bit_word_b & 0x1ff; _16bit_word_b >>= 9;
  _unpacked_block_event_info -> _RPC |= (_16bit_word_b & 0x3f) << 12;
 
  _unpacked_block_event_info -> _ME4  =  _16bit_word_a & 0x1ff; _16bit_word_a >>= 9;
  _unpacked_block_event_info -> _RPC |= (_16bit_word_a & 0x3f) << 18;
